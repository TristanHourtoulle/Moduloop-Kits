export function translateAuthError(error: string | Error | unknown): string {
  ...
  const errorTranslations: Record<string, string> = {
    ...
    "Account exists with different credential": "Un compte existe avec cette adresse e-mail mais utilise une méthode de connexion différente (Google au lieu d'email/mot de passe)",
    "Email already linked to Google account": "Cette adresse e-mail est déjà liée à un compte Google. Veuillez vous connecter avec Google.",
    "Account linked to social provider": "Ce compte est lié à Google. Veuillez vous connecter avec Google au lieu d'utiliser email/mot de passe.",
  };
  ...
  if (lowerMessage.includes('wrong') || lowerMessage.includes('incorrect') || lowerMessage.includes('invalid')) {
...
export function getSpecificAuthError(error: string | Error | unknown, context: 'signin' | 'signup' | 'google'): string {
