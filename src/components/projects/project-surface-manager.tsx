...
export function ProjectSurfaceManager({
  ...
  const hasChanges =
    isManual !== isOverride ||
    (isManual &&
      manualValue !== (manualSurface !== null && manualSurface !== undefined ? manualSurface : currentSurface));

  return (
    <div className='space-y-4 p-6 border rounded-lg bg-card shadow-sm'>
      <div>
        <Label className='text-sm font-semibold text-foreground mb-3 block'>Mode de calcul de la surface</Label>
        ...
        {isManual ? (
          <div className='space-y-2'>
            ...
            <p className='text-xs text-muted-foreground'>
              Surface calculée automatiquement : <span className='font-medium text-foreground'>{currentSurface.toFixed(2)} m²</span>
            </p>
          </div>
        ) : (
          <div className='space-y-2'>
            <p className='text-sm text-muted-foreground'>
              ...
              <span className='font-semibold text-foreground text-lg'>{currentSurface.toFixed(2)} m²</span>
            </p>
          </div>
        )}
